var rudderanalytics = (function (exports) {
  ...
  var CONFIG_URL = "https://api.rudderlabs.com/sourceConfig/?p=web&v=1.1.3";
  ...
  var Storage = /*#__PURE__*/function () {
    ...
    _createClass(Storage, [{
      ...
      value: function decryptValue(value) {
        if (!value || typeof value === "string" && this.trim(value) === "" || typeof value !== "string") {
          return value;
        } // value = this.parse(value);


        while (value.indexOf(defaults$1.prefix) >= 0) {
          var index = value.indexOf(defaults$1.prefix);
          var substring = value.substring(index + defaults$1.prefix.length);
          var dvalue = aes.decrypt(substring, defaults$1.key).toString(encUtf8);
          value = dvalue;
        }
      ...
      key: "setUserId",
      value: function setUserId(value, migrate) {
        if (typeof value !== "string") {
          logger.error("[Storage] setUserId:: userId should be string");
          return;
        }

        this.storage.set(migrate ? defaults$1.user_storage_key : v1Defaults.user_storage_key, this.encryptValue(this.stringify(value)));
      }
      /**
       *
       * @param {*} value
      ...
      key: "setUserTraits",
      value: function setUserTraits(value, migrate) {
        this.storage.set(migrate ? defaults$1.user_storage_trait : v1Defaults.user_storage_trait, this.encryptValue(this.stringify(value)));
      }
      /**
       *
       * @param {*} value
      ...
      key: "setGroupId",
      value: function setGroupId(value, migrate) {
        if (typeof value !== "string") {
          logger.error("[Storage] setGroupId:: groupId should be string");
          return;
        }

        this.storage.set(migrate ? defaults$1.group_storage_key : v1Defaults.group_storage_key, this.encryptValue(this.stringify(value)));
      }
      /**
       *
       * @param {*} value
      ...
      key: "setGroupTraits",
      value: function setGroupTraits(value, migrate) {
        this.storage.set(migrate ? defaults$1.group_storage_trait : v1Defaults.group_storage_trait, this.encryptValue(this.stringify(value)));
      }
      /**
       *
       * @param {*} value
      ...
      key: "setAnonymousId",
      value: function setAnonymousId(value, migrate) {
        if (typeof value !== "string") {
          logger.error("[Storage] setAnonymousId:: anonymousId should be string");
          return;
        }

        this.storage.set(migrate ? defaults$1.user_storage_anonymousId : v1Defaults.user_storage_anonymousId, this.encryptValue(this.stringify(value)));
      }
      /**
       *
       * @param {*} key
      ...
      value: function getItem(key) {
        ...
        var decryptedValue = this.decryptValue(item);
        var parsedValue = this.parse(decryptedValue);
        return parsedValue;
      }
      /**
      ...
      key: "getUserId",
      value: function getUserId(migrate) {
        var userId = this.storage.get(migrate ? defaults$1.user_storage_key : v1Defaults.user_storage_key);
        var decryptedValue = this.decryptValue(userId);
        var parsedValue = this.parse(decryptedValue);
        return parsedValue;
      }
      /**
      ...
      key: "getUserTraits",
      value: function getUserTraits(migrate) {
        var traits = this.storage.get(migrate ? defaults$1.user_storage_trait : v1Defaults.user_storage_trait);
        var decryptedValue = this.decryptValue(traits);
        var parsedValue = this.parse(decryptedValue);
        return parsedValue;
      }
      /**
      ...
      key: "getGroupId",
      value: function getGroupId(migrate) {
        var groupId = this.storage.get(migrate ? defaults$1.group_storage_key : v1Defaults.group_storage_key);
        var decryptedValue = this.decryptValue(groupId);
        var parsedValue = this.parse(decryptedValue);
        return parsedValue;
      }
      /**
      ...
      key: "getGroupTraits",
      value: function getGroupTraits(migrate) {
        var groupTraits = this.storage.get(migrate ? defaults$1.group_storage_trait : v1Defaults.group_storage_trait);
        var decryptedValue = this.decryptValue(groupTraits);
        var parsedValue = this.parse(decryptedValue);
        return parsedValue;
      }
      /**
      ...
      key: "getAnonymousId",
      value: function getAnonymousId(migrate) {
        var anonymousId = this.storage.get(migrate ? defaults$1.user_storage_anonymousId : v1Defaults.user_storage_anonymousId);
        var decryptedValue = this.decryptValue(anonymousId);
        var parsedValue = this.parse(decryptedValue);
        return parsedValue;
      }
      /**
      ...
      value: function clear() {
        ...
        this.storage.remove(v1Defaults.group_storage_trait); // this.storage.remove(defaults.user_storage_anonymousId);
      }
    }, {
      key: "getPrefix",
      value: function getPrefix() {
        return defaults$1.prefix;
      }
    }]);
  ...
  function validatePayload(message) {
    var prefix = Storage$1.getPrefix(); // "RudderEncrypt:";

    var traits = message.context.traits;
    var _message = message,
        userId = _message.userId,
        anonymousId = _message.anonymousId;

    if (traits && _typeof(traits) !== "object") {
      message = undefined;
    }
    ...
    if (anonymousId && typeof anonymousId === "string" && anonymousId.indexOf(prefix) >= 0) {
      message = undefined;
    }
  ...
  */
  ...
  var Analytics = /*#__PURE__*/function () {
    ...
    _createClass(Analytics, [{
      ...
      value: function initializeUser() {
        var migrate = this.storage.checkIfMigrate();
        this.userId = this.storage.getUserId(migrate) != undefined ? this.storage.getUserId(migrate) : "";
        this.userTraits = this.storage.getUserTraits(migrate) != undefined ? this.storage.getUserTraits(migrate) : {};
        this.groupId = this.storage.getGroupId(migrate) != undefined ? this.storage.getGroupId(migrate) : "";
        this.groupTraits = this.storage.getGroupTraits(migrate) != undefined ? this.storage.getGroupTraits(migrate) : {};
        this.anonymousId = this.getAnonymousId(migrate); // save once for storing older values to encrypted
      ...
      key: "merge",
      value: function merge(mergeObj, options, callback) {
        if (!this.loaded) return;

        if (_typeof(mergeObj) !== "object") {
        ...
        if (isNil(mergeProperties.identifier1.type) || isNil(mergeProperties.identifier1.value) || isNil(mergeProperties.identifier2.type) || isNil(mergeProperties.identifier2.value)) {
          logger.error("mergeProperties contains null values for expected inputs");
          return;
        }

        var rudderElement = new RudderElementBuilder().setType("merge").build();
        rudderElement.message.mergeProperties = mergeProperties;
        this.processAndSendDataToDestinations("merge", rudderElement, options, callback);
      }
      /**
       *
       * @param {*} to
      ...
      key: "setAnonymousId",
      value: function setAnonymousId(anonymousId, migrate) {
        // if (!this.loaded) return;
  ...
  var merge = instance.merge.bind(instance);
