import { parse, isAbsolute, isRelative, isCrossDomain } from '../../../src/npmPackages/component-url/index';
...
describe('isCrossDomain(str)', () => {
  describe('when everything matches', () => {
    it('should return false', () => {
      ...
      expect(isCrossDomain(`${window.location.protocol  }//${  window.location.host  }/hello/world`)).toBeFalsy();
    })
  })

  describe('when hostname mismatches', () => {
    it('should return true', () => {
      expect(isCrossDomain(`${window.location.protocol}//example.com${window.location.port ? `:${window.location.port}` : ''}/hello/world`)).toBeTruthy();
    })
  })
  ...
  describe('when port mismatches', () => {
    it('should return true', () => {
      expect(isCrossDomain(`${window.location.protocol}//${window.location.hostname}:${+window.location.port + 1}/hello/world`)).toBeTruthy();
    })
  })

  describe('when port is missing', () => {
    it('should return true', () => {
      expect(isCrossDomain(`${window.location.protocol}//${window.location.port}/hello/world`)).toBeTruthy();
    })
  })
})
