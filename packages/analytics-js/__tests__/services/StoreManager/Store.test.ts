...
describe('Store', () => {
  ...
  describe('.get', () => {
    ...
    it('should drop the entry and report only once if the value cannot be parsed', () => {
      ...
      // set() writes two JSON layers and get() strips both, so one layer short throws on parse.
    ...
    it('should leave the stored value in place when decryption fails', () => {
      ...
      expect(store.get(QueueStatuses.RECLAIM_START)).toBeNull();
      // A transient plugin failure must not cost the value; it reads fine once the plugin recovers.
    ...
    it('should drop an unparseable value that the engine deserializes on every read', () => {
      ...
      // storejs deserializes on every read, so a reference compare would never match this array.
    ...
    it.each([
      ...
      (_scenario, failedPlugins) => {
        ...
        // With no decrypt extension point registered, crypto hands the ciphertext straight
        // back, so it looks retrieved and then fails to parse. This is the real fallback.
    ...
    it('should keep an encrypted value that the configured plugin cannot decrypt', () => {
      ...
      const reclaimStartKey = `name.id.${QueueStatuses.RECLAIM_START}`;
      // A v1 value in a v3 store: v3 decrypt only handles its own prefix and returns anything
      // else untouched, so the ciphertext reaches JSON.parse.
