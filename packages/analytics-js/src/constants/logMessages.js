...
const PLUGIN_DEPS_ERROR = (context, pluginName, notExistDeps) =>
...
const PLUGIN_INVOCATION_ERROR = (context, extPoint, pluginName) =>
  `${context}${LOG_CONTEXT_SEPARATOR}Failed to invoke the "${extPoint}" extension point of plugin "${pluginName}".`;
const STORAGE_UNAVAILABILITY_ERROR_PREFIX = (context, storageType) =>
...
const READY_API_CALLBACK_ERROR = context =>
  `${context}${LOG_CONTEXT_SEPARATOR}The callback is not a function.`;
const XHR_DELIVERY_ERROR = (prefix, status, statusText, url) =>
  `${prefix} with status: ${status}, ${statusText} for URL: ${url}.`;
const XHR_REQUEST_ERROR = (prefix, e, url) =>
...
// WARNING
const STORAGE_TYPE_VALIDATION_WARNING = (context, storageType, defaultStorageType) =>
...
const UNSUPPORTED_ERROR_REPORTING_PROVIDER_WARNING = (
  context,
  selectedErrorReportingProvider,
...
const UNSUPPORTED_STORAGE_ENCRYPTION_VERSION_WARNING = (
  context,
  selectedStorageEncryptionVersion,
...
const RESERVED_KEYWORD_WARNING = (context, property, parentKeyPath, reservedElements) =>
...
const INVALID_CONTEXT_OBJECT_WARNING = logContext =>
...
const TIMEOUT_ZERO_WARNING = context =>
...
const TIMEOUT_NOT_RECOMMENDED_WARNING = (context, timeout, minTimeout) =>
...
const INVALID_SESSION_ID_WARNING = (context, sessionId, minSessionIdLength) =>
...
const STORAGE_QUOTA_EXCEEDED_WARNING = context =>
...
const STORAGE_UNAVAILABLE_WARNING = (context, selectedStorageType, finalStorageType) =>
...
const WRITE_KEY_NOT_A_STRING_ERROR = (context, writeKey) =>
...
const EMPTY_GROUP_CALL_ERROR = context =>
...
const UNSUPPORTED_PRE_CONSENT_STORAGE_STRATEGY = (context, selectedStrategy, defaultStrategy) =>
