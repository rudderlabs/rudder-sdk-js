...
class CapabilitiesManager implements ICapabilitiesManager {
  ...
  prepareBrowserCapabilities() {
    ...
    if (shouldLoadPolyfill) {
      ...
      // A timed out script is not removed from the DOM, so a response that arrives later can
      // still invoke the service callback. Settle the load once and drop the global handler
      ...
      const settlePolyfillLoad = (onSettled: () => void): void => {
        // Remove the entry from window so we don't leave room for calling it again, but
        // only while it is still the exact function this invocation installed. The name is
        // keyed by write key alone, so the global may belong to the page or to another
        // in-flight load, either from the outset or because it replaced ours since.
        if (
      ...
      this.externalSrcLoader.loadJSFile({
        ...
        callback: (scriptId?: string, err?: SDKError) => {
          if (!scriptId || err) {
            // The SDK needs these APIs to behave correctly, so the load is not
            // advanced past this point. Report the failure and stop here rather
            // than run on an engine that is still missing them.
            settlePolyfillLoad(() => {
