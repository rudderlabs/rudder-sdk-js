 ...
 * @return {?Object}
 */
const parseLinkerParamValue = value => {
  ...
  return {
    checksum: checksum !== null && checksum !== void 0 ? checksum : '',
    serializedIds,
  };
};
/**
 ...
 */
const deserialize = serializedIds => {
  const keyValuePairs = {};
 ...
 * @return {string}
 */
const getFingerprint = (userAgent, language) => {
  const date = new Date();
 ...
 * @return {string}
 */
const getCheckSum = (serializedIds, optOffsetMin, userAgent, language) => {
 ...
 * @return {boolean}
 */
const isCheckSumValid = (serializedIds, checksum) => {
