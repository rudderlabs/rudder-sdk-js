import {constructMessageIntegrationsObj, getMergedClientSuppliedIntegrations} from "../../src/utils/IntegrationsData";
...
describe('IntegrationsData utilities', () => {
    it('should construct message integrations object with integrations specific data attached', () => {
        const mergedArray = constructMessageIntegrationsObj(mockIntegrationsData, mockIntegrationInstances);
        expect(mergedArray).toEqual(expectedEnhancedIntegrationsObject);
    });

    it('should merge data for Client Supplied Integrations', () => {
        const mergedArray = getMergedClientSuppliedIntegrations(mockEnhancedIntegrationsData, mockClientSuppliedIntegrations);
    ...
    it('should merge data for Client Supplied Integrations keeping any client defined keys', () => {
        const mergedArray = getMergedClientSuppliedIntegrations(mockEnhancedIntegrationsData, mockClientSuppliedIntegrationsWithKeys);
    ...
    it('should merge data for Client Supplied Integrations keeping any client inactive integrations', () => {
        const mergedArray = getMergedClientSuppliedIntegrations(mockEnhancedIntegrationsData, mockClientSuppliedIntegrationsWithInactive);
