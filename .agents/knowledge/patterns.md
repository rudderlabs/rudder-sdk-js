...
- OpenAI Ads device-mode user clearing is integration-local: track the current user ID inside the destination, detect user ID changes or logout before `identify`/`track`/`page`/`screen`, clear stored Measurement Pixel user state with `window.oaiq("init", { pixelId, user: {} })`, then repopulate when new user data exists; reinitializing with only `{ pixelId }` does not clear vendor-side user state (`packages/analytics-js-integrations/src/integrations/OpenAIAds/browser.js`).
