...
- OpenAI Ads SDK device-mode event payload logic keeps browser validation local and limited: require configured `eventMapping` rows instead of falling back to exact standard names, preserve `source_url` from `properties.source_url`/`sourceUrl`/`context.page.url`, normalize mapped user fields, and let the native destination queue handle track allow/deny filtering before integration invocation (`packages/analytics-js-integrations/src/integrations/OpenAIAds/browser.js`, `packages/analytics-js-plugins/src/nativeDestinationQueue/utilities.ts`).
