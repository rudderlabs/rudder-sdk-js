...
- OpenAI Ads SDK device-mode event payload logic mirrors the transformer business rules: require configured `eventMapping` rows instead of falling back to exact standard names, preserve `source_url` from `properties.source_url`/`sourceUrl`/`context.page.url`, validate `action_source`, `currency`, `opt_out`, and non-scalar dedupe fields consistently, emit default currency only with amount, and include `amount`/`currency`/`contents` for `plan_enrollment` where the pixel payload supports `eventData` fields (`packages/analytics-js-integrations/src/integrations/OpenAIAds/browser.js`).
