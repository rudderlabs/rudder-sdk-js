...
## INT-7067 — OpenAI Ads Measurement Pixel Handling

- OpenAI Ads device-mode does not read or forward the `__obref` cookie, `traits.obref`, or `context.traits.obref` from SDK code; the Measurement Pixel owns browser attribution cookies and CAPI-only `obref` handling stays out of the pixel integration (`packages/analytics-js-integrations/src/integrations/OpenAIAds/browser.js`).
- OpenAI Ads device-mode user clearing is integration-local: track the current user ID inside the destination, detect user ID changes or logout before `identify`/`track`/`page` (the web SDKs have no `screen` API, so device-mode integrations must not define one), clear stored Measurement Pixel user state with `window.oaiq("init", { pixelId, user: {} })`, then repopulate when new user data exists; reinitializing with only `{ pixelId }` does not clear vendor-side user state (`packages/analytics-js-integrations/src/integrations/OpenAIAds/browser.js`).
- OpenAI Ads SDK device-mode event payload logic stays pixel-specific: require configured `eventMapping` rows instead of falling back to exact standard names; leave browser attribution fields such as `source_url`, `action_source`, and `oppref` to the pixel/server-side paths; put only documented pixel event-data fields in `eventData` (`type`, `amount`, `currency`, `contents`, `plan_id`); send `event_id` / `custom_event_name` / `opt_out` in event options; normalize mapped user fields to singular scalar pixel user fields; and let the native destination queue handle track allow/deny filtering before integration invocation (`packages/analytics-js-integrations/src/integrations/OpenAIAds/browser.js`, `packages/analytics-js-plugins/src/nativeDestinationQueue/utilities.ts`).
- OpenAI Ads event-data handling should emit default currency only when an amount is present and include `plan_id` for `plan_enrollment` and custom events where the pixel payload supports it (`packages/analytics-js-integrations/src/integrations/OpenAIAds/browser.js`).
...
- MoEngage web device mode maps canonical regions `US`, `EU`, `IND`, `US-DC-04`, `SGP-DC-05`, and `IDN-DC-06` to `dc_1` through `dc_6`, while retaining legacy `IN` as an alias for India's `dc_3` (`packages/analytics-js-integrations/src/integrations/MoEngage/utils.js`).
- Empty regions preserve the legacy silent `dc_1` fallback; non-empty unknown regions warn and then default to `dc_1`. The utility uses a module-local `Logger(DISPLAY_NAME)` explicitly set to `WARN`, because the legacy logger defaults to `ERROR` and would otherwise suppress the warning (`packages/analytics-js-integrations/src/integrations/MoEngage/utils.js`).
- Region lookup uses `Object.prototype.hasOwnProperty.call` so inherited object properties such as `toString` are treated as unknown and cannot become invalid native-loader data-center arguments (`packages/analytics-js-integrations/src/integrations/MoEngage/utils.js`).
