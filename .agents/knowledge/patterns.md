...
- OpenAI Ads device-mode does not read or forward the `__obref` cookie from SDK code; the Measurement Pixel owns that cookie, while explicit `traits.obref` / `context.traits.obref` continue through the normal user-data mapping path (`packages/analytics-js-integrations/src/integrations/OpenAIAds/browser.js`).
...
- OpenAI Ads SDK device-mode event payload logic keeps browser validation local and limited: require configured `eventMapping` rows instead of falling back to exact standard names, preserve `source_url` from `properties.source_url`/`sourceUrl`/`context.page.url`, validate `action_source`/`currency`/`opt_out`/non-scalar dedupe values consistently, normalize mapped user fields, and let the native destination queue handle track allow/deny filtering before integration invocation (`packages/analytics-js-integrations/src/integrations/OpenAIAds/browser.js`, `packages/analytics-js-plugins/src/nativeDestinationQueue/utilities.ts`).
- OpenAI Ads event-data handling should emit default currency only when an amount is present and include amount/currency/contents for `plan_enrollment` where the pixel payload supports eventData fields (`packages/analytics-js-integrations/src/integrations/OpenAIAds/browser.js`).
