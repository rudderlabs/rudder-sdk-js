...
GA360.prototype.loadScript = jest.fn();

describe("GA360 init tests", () => {
  let googleAnalytics;
  beforeEach(() => {
    googleAnalytics = new GA360({ trackingID: "UA-143161493-8" }, {loadIntegration: true});
    googleAnalytics.init();
  });

  test("GA360 init default config test time", () => {
    // const googleAnalytics = new GA360({ trackingID: "UA-143161493-8" });
    // googleAnalytics.init();
    expect(typeof window.ga.l).toBe("number");
  });

  test("GA360 init default config test create and require", () => {
 ...
 logger.debug(JSON.stringify(window.ga.mock.calls));
  ...
  describe("GA360 simple non ecomm event", () => {
    let googleAnalytics;
    beforeEach(() => {
      googleAnalytics = new GA360({
        trackingID: "UA-143161493-8",
