...
class Analytics {
  ...
  replayEvents(object) {
    if (
      object.successfullyLoadedIntegration.length + object.failedToBeLoadedIntegration.length ===
      ...
      if (object.clientIntegrationObjects.every((intg) => !intg.isReady || intg.isReady())) {
        ...
        // set clientIntegrationsReady to be true
        object.integrationsData = constructMessageIntegrationsObj(
          object.integrationsData,
          object.clientIntegrationObjects,
        );
  ...
  processOptionsParam(rudderElement, options) {
    ...
    rudderElement.message.context.page =
      type == 'page' ? this.getContextPageProperties(properties) : this.getContextPageProperties();
  ...
  loadAfterPolyfill(writeKey, serverUrl, options) {
    ...
    if (options && options.sendAdblockPageOptions && typeof options.sendAdblockPageOptions === 'object') {
    ...
    if (options && options.clientSuppliedCallbacks) {
      ...
      Object.keys(this.methodToCallbackMapping).forEach((methodName) => {
        if (this.methodToCallbackMapping.hasOwnProperty(methodName) && options.clientSuppliedCallbacks[this.methodToCallbackMapping[methodName]]) {
  ...
  registerCallbacks(calledFromLoad) {
    if (!calledFromLoad) {
      Object.keys(this.methodToCallbackMapping).forEach((methodName) => {
        if (this.methodToCallbackMapping.hasOwnProperty(methodName) && window.rudderanalytics && 
