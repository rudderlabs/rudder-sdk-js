...
class Analytics {
  ...
  initializeSession(options) {
    ...
    if (options && typeof options.sessionTimeout === 'number'){
      ...
      if (options.sessionTimeout === 0) {
        logger.warn('[Session]:: Provided sessionTimeout value 0 will disable the auto session tracking feature.');
  ...
  processAndSendDataToDestinations(type, rudderElement, options, callback) {
    try {
      ...
      if (this.autoTrackSession || this.sessionInfo.manuallyTrackSession) {
        try {
          const timestamp = Date.now();
          rudderElement.message.context.sessionId = this.getSessionId(timestamp);
