const rudderanalytics = (function (exports) {
  ...
  const componentEmitter = createCommonjsModule((module) => {
    /**
     * Expose `Emitter`.
     */
    {
  ...
  const trim_1 = createCommonjsModule((module, exports) => {
  ...
  const trim_2 = trim_1.left;
  const trim_3 = trim_1.right;
  ...
  const componentType = function componentType(val) {
  ...
  const stringify = function stringify(obj) {
    ...
    for (const key in obj) {
      ...
      if (componentType(value) == 'array') {
        for (const [i, element] of value.entries()) {
          pairs.push(`${encode(`${key  }[${  i  }]`)  }=${  encode(element)}`);
        }
  ...
  const componentQuerystring = {
    parse,
    stringify,
  };

  const lodash_merge = createCommonjsModule((module, exports) => {
    /**
  ...
  const foldl = function foldl(iterator, accumulator, collection) {
    if (typeof iterator !== 'function') {
      throw new TypeError(`Expected a function but received a ${  _typeof(iterator)}`);
    }

    each_1((val, i, collection) => {
  ...
  const parse$1 = componentQuerystring.parse;
  /**
  ...
  function utm(query) {
    ...
    let param;
    const params = parse$1(query);
    const results = {};

    for (const key in params) {
      if (has$1.call(params, key) && key.substr(0, 4) === 'utm_') {
          param = key.substr(4);
  ...
  const lib = utm;
  const strict_1 = strict;
  lib.strict = strict_1;

  const componentUrl = createCommonjsModule((module, exports) => {
    /**
     * Parse the given `url`.
     *
     * @param {String} str
  ...
  const CONFIG_URL = 'https://api.rudderlabs.com/sourceConfig/?p=web&v=1.1.9';
  ...
  function getJSONTrimmed(context, url, writeKey, callback) {
    ...
    xhr.onload = function () {
      ...
      if (status == 200) {
        ...
        handleError(
          new Error('request failed with status: '.concat(xhr.status, ' for url: ').concat(url)),
        );
        cb_(status);
      }
    };
  ...
  function handleError(error, analyticsInstance) {
    ...
    try {
      if (error instanceof Event && error.target && error.target.localName == 'script') {
          errorMessage = 'error in script loading:: src::  '
            .concat(error.target.src, ' id:: ')
            .concat(error.target.id);
  ...
  function getDefaultPageProperties() {
    ...
    const path = canonicalUrl ? componentUrl_1(canonicalUrl).pathname : window.location.pathname;
    const _document = document;
      const {referrer} = _document;
    const {search} = window.location;
    const _document2 = document;
      const {title} = _document2;
  ...
  const HubSpot = /* #__PURE__ */ (function () {
    ...
    _createClass(HubSpot, [
      {
        key: 'init',
        value: function init() {
          const hubspotJs = 'https://js.hs-scripts.com/'.concat(this.hubId, '.js');
  ...
  const toStr$1 = objProto.toString;
  let symbolValueOf;
  ...
  const isActualNaN = function isActualNaN(value) {
    return value !== value;
  };
  ...
  const base64Regex = /^([\d+/A-Za-z]{4})*([\d+/A-Za-z]{4}|[\d+/A-Za-z]{3}=|[\d+/A-Za-z]{2}==)$/;
  ...
  is.args = is.arguments = function (value) {
    const isStandardArguments = toStr$1.call(value) === '[object Arguments]';
  ...
  is.within = function (value, start, finish) {
    ...
    const isAnyInfinite = is.infinite(value) || is.infinite(start) || is.infinite(finish);
  ...
  is.base64 = function (value) {
    return is.string(value) && (value.length === 0 || base64Regex.test(value));
  };
  /**
   * Test base64 string.
   */
  ...
  is.hex = function (value) {
    return is.string(value) && (value.length === 0 || hexRegex.test(value));
  };
  /**
   * is.symbol
  ...
  is.symbol = function (value) {
    return (
      typeof Symbol === 'function' &&
      toStr$1.call(value) === '[object Symbol]' &&
      _typeof(symbolValueOf.call(value)) === 'symbol'
    );
  };
  /**
   * is.bigint
  ...
  is.bigint = function (value) {
    ...
    return (
      ...
      toStr$1.call(value) === '[object BigInt]' &&
  ...
  const componentType$1 = function componentType(val) {
  ...
  const componentProps = function componentProps(str, fn) {
  ...
  function unique(arr) {
    ...
    for (const element of arr) {
      if (~ret.indexOf(element)) continue;
      ret.push(element);
    }
  ...
  function get(str) {
    ...
    if (props.length === 0) return `_.${  str}`;
    let val; let i; let prop;
  ...
  function stripNested(prop, str, val) {
    return str.replace(new RegExp(`(\\.)?${  prop}`, 'g'), ($0, $1) => $1 ? $0 : val);
  }
  ...
  const componentEach = function componentEach(obj, fn, ctx) {
    fn = toFunction_1(fn);
    ctx = ctx || this;
    ...
    _createClass(GA, [
      ...
      },
      {
        ...
        value: function track(rudderElement) {
          ...
          const _rudderElement$messag = rudderElement.message;
            const {event} = _rudderElement$messag;
            const {properties} = _rudderElement$messag;
            const {name} = _rudderElement$messag;
            ...
            switch (event) {
              ...
              case 'Product List Filtered':
                ...
                filters = props.filters
                  .map((obj) => ''.concat(obj.type, ':').concat(obj.value))
                  .join();
                sorts = props.sorters
                  .map((obj) => ''.concat(obj.type, ':').concat(obj.value))
                  .join();
      ...
      },
      {
        ...
        value: function page(rudderElement) {
          ...
          if (rudderElement.message.properties.category && rudderElement.message.name) {
            name = ''
              .concat(rudderElement.message.properties.category, ' ')
              .concat(rudderElement.message.name);
      ...
      },
      {
        ...
        value: function pushEnhancedEcommerce(rudderElement) {
          ...
          (_window$ga = window.ga).call.apply(
            _window$ga,
            [window].concat(_toConsumableArray(args)),
          );
        },
        /**
         * @param  {} item
  ...
  const Hotjar = /* #__PURE__ */ (function () {
  ...
  const GoogleAds = /* #__PURE__ */ (function () {
    ...
    _createClass(GoogleAds, [
      {
        key: 'init',
        value: function init() {
          const sourceUrl = 'https://www.googletagmanager.com/gtag/js?id='.concat(this.conversionId);
      ...
      },
      {
        ...
        value: function track(rudderElement) {
          ...
          if (conversionData.conversionLabel) {
            ...
            const sendToValue = ''.concat(this.conversionId, '/').concat(conversionLabel);
            const properties = {};
              ...
              init: function init() {
                ...
                this.load(
                  '//dev.visualwebsiteoptimizer.com/j.php?a='
                    .concat(account_id, '&u=')
                    .concat(encodeURIComponent(d.URL), '&r=')
                    .concat(Math.random(), '&f=')
                    .concat(+isSPA),
                );
  ...
  const GoogleTagManager = /* #__PURE__ */ (function () {
  ...
  */
    ...
    _createClass(Braze, [
      {
        ...
        value: function formatGender(gender) {
          ...
          if (femaleGenders.includes(gender.toLowerCase()))
            return window.appboy.ab.User.Genders.FEMALE;
          if (maleGenders.includes(gender.toLowerCase()))
            return window.appboy.ab.User.Genders.MALE;
          if (otherGenders.includes(gender.toLowerCase()))
  ...
  const crypt = createCommonjsModule((module) => {
    (function () {
      const base64map = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
        var crypt = {
          ...
          base64ToBytes: function base64ToBytes(base64) {
            ...
            base64 = base64.replace(/[^\d+/a-z]/gi, '');

            for (var bytes = [], i = 0, imod4 = 0; i < base64.length; imod4 = ++i % 4) {
              if (imod4 == 0) continue;
              bytes.push(
                ((base64map.indexOf(base64.charAt(i - 1)) & (2**(-2 * imod4 + 8) - 1)) <<
                  (imod4 * 2)) |
  ...
  const isBuffer_1 = function isBuffer_1(obj) {
  ...
  const md5 = createCommonjsModule((module) => {
    (function () {
      const crypt$1 = crypt;
        ...
        const {bin} = charenc_1;
        // The core
        const md5 = function md5(message, options) {
  ...
  const INTERCOM = /* #__PURE__ */ (function () {
  ...
  const objCase = createCommonjsModule((module) => {
    /**
    ...
    function multiple(fn) {
      return function (obj, path, val, options) {
          ...
          for (key in obj) {
            ...
            if (path.indexOf(normalizedKey) === 0) {
              ...
              if (temp.charAt(0) === '.' || temp.length === 0) {
                path = temp.substr(1);
                const child = obj[key]; // we're at the end and there is nothing.
        ...
        if (!key) return;
        if (obj == null) return obj; // the `obj` and `key` is one above the leaf object and key, so
    ...
    function defaultNormalize(path) {
      return path.replace(/[^\d.A-Za-z]+/g, '').toLowerCase();
    }
    /**
  ...
  const Kissmetrics = /* #__PURE__ */ (function () {
    ...
    _createClass(Kissmetrics, [
      ...
      },
      {
        ...
        value: function prefix(event, properties) {
          const prefixed = {};
          componentEach(properties, (key, val) => {
  ...
  const CustomerIO = /* #__PURE__ */ (function () {
  ...
  const Chartbeat = /* #__PURE__ */ (function () {
    ...
    _createClass(Chartbeat, [
      ...
      },
      {
        ...
        value: function initAfterPage() {
          ...
          onBody(() => {
            const script = _this.isVideo ? 'chartbeat_video.js' : 'chartbeat.js';
      ...
      },
      {
        ...
        value: function _isReady(instance) {
          ...
          return new Promise((resolve) => {
            ...
            _this2.pause(INTEGRATION_LOAD_CHECK_INTERVAL).then(() => _this2
                ._isReady(instance, time + INTEGRATION_LOAD_CHECK_INTERVAL)
                .then(resolve));
          });
        },
      },
  ...
  const Comscore = /* #__PURE__ */ (function () {
      ...
      },
      {
        ...
        value: function _isReady(instance) {
          ...
          return new Promise((resolve) => {
            ...
            _this.pause(INTEGRATION_LOAD_CHECK_INTERVAL).then(() => _this._isReady(instance, time + INTEGRATION_LOAD_CHECK_INTERVAL).then(resolve));
          });
        },
      },
      {
  ...
  const FacebookPixel = /* #__PURE__ */ (function () {
            ...
            each_1((event) => {
              window.fbq(
                ...
                event,
                {
                  currency: rudderElement.message.properties.currency,
                  value: useValue
                    ? _this.formatRevenue(rudderElement.message.properties.value)
                    : _this.formatRevenue(rudderElement.message.properties.price),
                },
                {
                  eventID: rudderElement.message.messageId,
                },
      ...
      },
      {
        ...
        value: function buildPayLoad(rudderElement, isStandardEvent) {
          ...
          for (const property in properties) {
            ...
            if (isStandardEvent && !eventCustomProperties.includes(property)) {
              continue;
            }

            const value = properties[property];

            if (dateFields.includes(properties) && is_1.date(value)) {
  ...
  const core = createCommonjsModule((module, exports) => {
  ...
  const encBase64 = createCommonjsModule((module, exports) => {
    (function (root, factory) {
      {
        // CommonJS
  ...
  const md5$1 = createCommonjsModule((module, exports) => {
    (function (root, factory) {
      {
        // CommonJS
        module.exports = exports = factory(core);
  ...
  const sha1 = createCommonjsModule((module, exports) => {
    (function (root, factory) {
      {
        // CommonJS
        module.exports = exports = factory(core);
  ...
  const hmac = createCommonjsModule((module, exports) => {
    (function (root, factory) {
      {
        // CommonJS
        module.exports = exports = factory(core);
  ...
  const evpkdf = createCommonjsModule((module, exports) => {
  ...
  const cipherCore = createCommonjsModule((module, exports) => {
    (function (root, factory, undef) {
      {
        // CommonJS
  ...
  const aes = createCommonjsModule((module, exports) => {
    (function (root, factory, undef) {
      {
        // CommonJS
          ...
          _doCryptBlock: function _doCryptBlock(
            ...
            for (let round = 1; round < nRounds; round++) {
              ...
              var t3 =
                ...
                SUB_MIX_2[(s1 >>> 8) & 0xff] ^
                SUB_MIX_3[s2 & 0xff] ^
                keySchedule[ksRow++]; // Update state
  ...
  const encUtf8 = createCommonjsModule((module, exports) => {
    (function (root, factory) {
      {
        // CommonJS
        module.exports = exports = factory(core);
  ...
  const componentType$2 = function componentType(val) {
  ...
  const debug_1 = createCommonjsModule((module, exports) => {
    /**
  ...
  const browser = createCommonjsModule((module, exports) => {
    /**
    ...
    function useColors() {
      ...
      return (
        'WebkitAppearance' in document.documentElement.style || // is firebug? http://stackoverflow.com/a/398120/376773
  ...
  const has$4 = Object.prototype.hasOwnProperty;
  ...
  const json3 = createCommonjsModule((module, exports) => {
    (function () {
      // Detect the `define` function exposed by asynchronous module loaders. The
      // strict `define` check is necessary for compatibility with `r.js`.
      const isLoader = typeof undefined === 'function'; // A set of types used to distinguish objects from primitives.
      ...
      const freeExports = objectTypes.object && exports && !exports.nodeType && exports; // Use the `global` object exposed by Node (including Browserify via
      // `insert-module-globals`), Narwhal, and Ringo as the default context,
      // and the `window` object in browsers. Rhino exports a `global` function
      // instead.

      let root =
          (objectTypes[typeof window === 'undefined' ? 'undefined' : _typeof(window)] && window) ||
          this;
        const freeGlobal =
          freeExports &&
          objectTypes.object &&
      ...
      function runInContext(context, exports) {
        ...
        const Number = context.Number || root.Number;
          ...
          const nativeJSON = context.JSON || root.JSON; // Delegate to the native `stringify` and `parse` implementations.
        ...
        const objectProto = Object.prototype;
          ...
          let undefined$1; // Internal: Contains `try...catch` logic used by other functions.
        ...
        function has(name) {
          ...
          if (name == 'bug-string-char-index') {
            ...
            let value;
              const serialized = '{"a":[1,true,false,null,"\\u0000\\b\\n\\f\\r\\t"]}'; // Test `JSON.stringify`.
        ...
        if (!has('json')) {
          ...
          const functionClass = '[object Function]';
            ...
            const booleanClass = '[object Boolean]'; // Detect incomplete support for accessing string characters by index.

          const charIndexBuggy = has('bug-string-char-index'); // Internal: Normalizes the `for...in` iteration algorithm across
          // environments. Each enumerated key is yielded to a `callback` function.

          let _forOwn = function forOwn(object, callback) {
            let size = 0;
              let Properties;
              let dontEnums;
              let property; // Tests for bugs in the current environment's `for...in` algorithm. The
              ...
              _forOwn = function forOwn(object, callback) {
                ...
                const hasProperty =
                  (!isFunction &&
                    ...
                    objectTypes[_typeof(object.hasOwnProperty)] &&
          ...
          if (!has('json-stringify') && !has('date-serialization')) {
            ...
            const toPaddedString = function toPaddedString(width, value) {
              // The `|| 0` expression is necessary to work around a bug in
              // Opera <= 7.54u2 where `0 == -0`, but `String(-0) !== "0"`.
              return (leadingZeroes + (value || 0)).slice(-width);
            }; // Internal: Serializes a date object.

            let _serializeDate = function serializeDate(value) {
              let getData; let year; let month; let date; let time; let hours; let minutes; let seconds; let milliseconds; // Define additional utility methods if the `Date` methods are buggy.

              if (!isExtended) {
                const {floor} = Math; // A mapping between the months of the year and the number of days between
                // January 1st and the first of the respective month.

                const Months = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334]; // Internal: Calculates the number of days between the Unix epoch and the
                // first day of the given month.

                const getDay = function getDay(year, month) {
                  return (
                    Months[month] +
                    365 * (year - 1970) +
            ...
            if (has('json-stringify') && !has('date-serialization')) {
              ...
              const dateToJSON = function dateToJSON(key) {
              ...
              const escapeChar = function escapeChar(character) {
              ...
              const serialize = function serialize(
                ...
                let value; let type; let className; let results; let element; let index; let length; let prefix; let result;
                attempt(() => {
                ...
                if (_typeof(value) == 'object') {
                  ...
                  if (className == arrayClass) {
                    ...
                    // itself.
                    _forOwn(properties || value, (property) => {
              ...
              exports.stringify = function (source, filter, width) {
                let whitespace; let callback; let properties; let className;
                ...
                if (objectTypes[_typeof(filter)] && filter) {
                  ...
                  if (className == functionClass) {
                    ...
                    for (var index = 0, {length} = filter, value; index < length; ) {
                      value = filter[index++];
                      className = getClass.call(value);
          ...
          if (!has('json-parse')) {
            const {fromCharCode} = String; // Internal: A map of escaped control characters and their unescaped
            // equivalents.
            ...
            let Index; let Source; // Internal: Resets the parser state and throws a `SyntaxError`.
            ...
            const lex = function lex() {
              ...
              while (Index < length) {
                ...
                switch (charCode) {
                  ...
                  default: // Advance past the negative sign, if one is specified.
                  ...
                  // Unrecognized token.
                    ...
                    if (charCode >= 48 && charCode <= 57) {
                      ...
                      for (
                        ;
                        Index < length &&
                        ((charCode = source.charCodeAt(Index)), charCode >= 48 && charCode <= 57);
                        Index++
                    ...
                    if (temp == 'true') {
                      Index += 4;
                      return true;
                    } if (temp == 'fals' && source.charCodeAt(Index + 4) == 101) {
                      Index += 5;
                      return false;
                    } if (temp == 'null') {
                      Index += 4;
            ...
            const get = function get(value) {
              let results; let hasMembers;
            ...
            const update = function update(source, property, callback) {
  ...
  const debug_1$1 = createCommonjsModule((module, exports) => {
    /**
     * This is the common logic for both the Node.js and web browser
     * implementations of `debug()`.
     *
  ...
  const browser$1 = createCommonjsModule((module, exports) => {
    /**
     * This is the web browser implementation of `debug()`.
     *
     * Expose `debug()` as the module.
  ...
  function set$1(name, value, options) {
    options = options || {};
    let str = `${encode$2(name)  }=${  encode$2(value)}`;
    if (value == null) options.maxage = -1;
    ...
    _createClass(CookieLocal, [
      {
        ...
        value: function options() {
          ...
          let domain = '.'.concat(lib$1(window.location.href));
  ...
  const store = (function () {
    // Store.js
    const store = {};
      const win = typeof window !== 'undefined' ? window : commonjsGlobal;
    ...
    if (isLocalStorageNameSupported()) {
      ...
      let storageOwner; let storageContainer; // Since #userData storage applies only to specific paths, we need to
      ...
      const withIEStorage = function withIEStorage(storeFunction) {
        return function () {
      ...
      const ieKeyFix = function ieKeyFix(key) {
        return key.replace(/^d/, '___$&').replace(forbiddenCharsRegex, '___');
      };

      store.set = withIEStorage((storage, key, val) => {
        key = ieKeyFix(key);
      ...
      store.get = withIEStorage((storage, key, defaultVal) => {
        key = ieKeyFix(key);
        const val = store.deserialize(storage.getAttribute(key));
        return val === undefined ? defaultVal : val;
      });
      store.remove = withIEStorage((storage, key) => {
        key = ieKeyFix(key);
        storage.removeAttribute(key);
        storage.save(localStorageName);
      });
      store.clear = withIEStorage((storage) => {
      ...
      store.forEach = withIEStorage((storage, callback) => {
        const {attributes} = storage.XMLDocument.documentElement;
  ...
  const Storage = /* #__PURE__ */ (function () {
    function Storage() {
    ...
    _createClass(Storage, [
      ...
      },
      {
        ...
        value: function decryptValue(value) {
          ...
          if (value.substring(0, defaults$1.prefix.length) == defaults$1.prefix) {
            return aes
              .decrypt(value.substring(defaults$1.prefix.length), defaults$1.key)
              .toString(encUtf8);
          }
      ...
      },
      {
        ...
        value: function setGroupTraits(value) {
          this.storage.set(
            defaults$1.group_storage_trait,
      ...
      },
      {
        ...
        value: function setAnonymousId(value) {
          ...
          this.storage.set(
            defaults$1.user_storage_anonymousId,
            this.encryptValue(this.stringify(value)),
          );
        },
        /**
      ...
      },
      {
        ...
        value: function getAnonymousId() {
          return this.parse(
            this.decryptValue(this.storage.get(defaults$1.user_storage_anonymousId)),
          );
        },
        /**
         *
  ...
  const LotameStorage = /* #__PURE__ */ (function () {
  ...
  const Lotame = /* #__PURE__ */ (function () {
  ...
  const Optimizely = /* #__PURE__ */ (function () {
    function Optimizely(config, analytics) {
      ...
      this.sendDataToRudder = function (campaignState) {
        ...
        const {audiences} = campaignState; // Reformatting this data structure into hash map so concatenating variation ids and names is easier later
        ...
        if (_this.sendExperimentTrack) {
          ...
          if (data && _this.customCampaignProperties.length > 0) {
            for (let index = 0; index < _this.customCampaignProperties.length; index += 1) {
              const rudderProp = _this.customCampaignProperties[index].from;
              const optimizelyProp = _this.customCampaignProperties[index].to;
    ...
    _createClass(Optimizely, [
      ...
      },
      {
        ...
        value: function initOptimizelyIntegration(referrerOverride, sendCampaignData) {
          const newActiveCampaign = function newActiveCampaign(id, referrer) {
          ...
          const checkReferrer = function checkReferrer() {
            const state = window.optimizely.get && window.optimizely.get('state');
          ...
          const registerFutureActiveCampaigns = function registerFutureActiveCampaigns() {
          ...
          const registerCurrentlyActiveCampaigns = function registerCurrentlyActiveCampaigns() {
            window.optimizely = window.optimizely || [];
            const state = window.optimizely.get && window.optimizely.get('state');
  ...
  const Bugsnag = /* #__PURE__ */ (function () {
  ...
  const preserveCamelCase = function preserveCamelCase(string, locale) {
  ...
  const camelcase = camelCase; // TODO: Remove this for the next major release

  const default_1 = camelCase;
  camelcase.default = default_1;

  const Fullstory = /* #__PURE__ */ (function () {
  ...
  const TVSquared = /* #__PURE__ */ (function () {
    ...
    _createClass(TVSquared, [
      ...
      },
      {
        ...
        value: function track(rudderElement) {
          const _rudderElement$messag = rudderElement.message;
            const {event} = _rudderElement$messag;
            const {userId} = _rudderElement$messag;
            const {anonymousId} = _rudderElement$messag;
          const _rudderElement$messag2 = rudderElement.message.properties;
            const {revenue} = _rudderElement$messag2;
            const {productType} = _rudderElement$messag2;
            const {category} = _rudderElement$messag2;
            const {order_id} = _rudderElement$messag2;
            const {promotion_id} = _rudderElement$messag2;
          let i;
          let j;
  ...
  */
  ...
  function getDestinationEventProperties(props, destParameterConfig) {
    const hasItem = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
  ...
  function getDestinationItemProperties(products, item) {
    ...
    if (type(products) !== 'array') {
      ...
      products.forEach((p) => {
        obj = _objectSpread2(
          _objectSpread2({}, getDestinationEventProperties(p, itemParametersConfigArray)),
    ...
    _createClass(GA4, [
      {
        ...
        value: function loadScript(measurementId, userId) {
          ...
          ScriptLoader(
            'google-analytics 4',
            'https://www.googletagmanager.com/gtag/js?id='.concat(measurementId),
          );
        },
      },
      {
  ...
  const MoEngage = /* #__PURE__ */ (function () {
          ...
          if (traits) {
            each_1((value, key) => {
              ...
              if (Object.prototype.hasOwnProperty.call(traitsMap, key)) {
                const method = 'add_'.concat(traitsMap[key]);
  ...
  const Amplitude = /* #__PURE__ */ (function () {
    ...
    _createClass(Amplitude, [
      ...
      },
      {
        ...
        value: function track(rudderElement) {
          ...
          const {properties} = rudderElement.message; // message.properties will always be present as part of track event.
      ...
      },
      {
        ...
        value: function trackingEventAndRevenuePerProduct(
          trackEventMessage,
          products,
          shouldTrackEventPerProduct,
        ) {
          const _trackEventMessage$pr = trackEventMessage.properties;
            const {revenue} = _trackEventMessage$pr;
            let {revenueType} = _trackEventMessage$pr;
            const {revenue_type} = _trackEventMessage$pr;
      ...
      },
      {
        ...
        value: function logEventAndCorrespondingRevenue(rudderMessage, dontTrackRevenue) {
          const {properties} = rudderMessage;
            const {event} = rudderMessage;
      ...
      },
      {
        ...
        value: function page(rudderElement) {
          ...
          const _rudderElement$messag = rudderElement.message;
            const {properties} = _rudderElement$messag;
            const {name} = _rudderElement$messag;
            const {category} = _rudderElement$messag; // all pages
          ...
          if (category && this.trackCategorizedPages) {
            const _event = 'Viewed page '.concat(category);

            amplitude.getInstance().logEvent(_event, properties);
          } // named pages

          if (name && this.trackNamedPages) {
            const _event2 = 'Viewed page '.concat(name);
      ...
      },
      {
        ...
        value: function group(rudderElement) {
          ...
          this.setDeviceId(rudderElement);
          const _rudderElement$messag2 = rudderElement.message;
            const {groupId} = _rudderElement$messag2;
            const {traits} = _rudderElement$messag2;
      ...
      },
      {
        ...
        value: function trackRevenue(rudderMessage) {
          ...
          let {price} = properties;
            let {productId} = properties;
            let {quantity} = properties;
            const {revenue} = properties;
            const {product_id} = properties;
          const revenueType =
    ...
    _createClass(Pendo, [
      ...
      },
      {
        ...
        value: function initializeMe() {
          ...
          const accountObj = _objectSpread2(
            {
          ...
          const visitorObj = _objectSpread2(
            {
              id: userId,
            },
      ...
      },
      {
        ...
        value: function group(rudderElement) {
          ...
          const _rudderElement$messag = rudderElement.message;
            const {userId} = _rudderElement$messag;
            const {traits} = _rudderElement$messag;
  ...
  const Lytics = /* #__PURE__ */ (function () {
    ...
    _createClass(Lytics, [
      {
        ...
        value: function loadLyticsScript() {
          (function () {
            const o = window.jstag || (window.jstag = {});
              let r = [];
            ...
            n('send'),
              ...
              (o.loadScript = function (n, t, i) {
                ...
                const o = document.getElementsByTagName('script')[0];
                  const r = (o && o.parentNode) || document.head || document.body;
                  const c = o || r.lastChild;
          ...
          window.jstag.init({
            ...
            src:
              document.location.protocal === 'https:'
                ? 'https://c.lytics.io/api/tag/'.concat(this.accountId, '/latest.min.js')
                : 'http://c.lytics.io/api/tag/'.concat(this.accountId, '/latest.min.js'),
          });
        },
      },
      {
  ...
  const Appcues = /* #__PURE__ */ (function () {
  ...
  function v4(options, buf, offset) {
    ...
    options = options || {};
    const rnds = options.random || (options.rng || rngBrowser)(); // Per 4.4, set bits for version and `clock_seq_hi_and_reserved`
  ...
  const defaultEngine = pickStorage(); // Expose the in-memory store explicitly for testing
  ...
  Store$1.prototype._createValidKey = function (key) {
    const {name} = this;
    const {id} = this;
    if (keys_1(this.keys).length === 0) return [name, id, key].join('.'); // validate and return undefined if invalid key
  ...
  const debug$3 = debug_1$2('localstorage-retry'); // Some browsers don't support Function.prototype.bind, so just including a simplified version here
  ...
  Queue.prototype._processHead = function () {
    ...
    while (queue.length > 0 && queue[0].time <= now && inProgressSize++ < self.maxItems) {
      ...
      const id = uuid$2(); // Save this to the in progress map
    ...
    _createClass(EventRepository, [
      {
        ...
        value: function startQueue(options) {
          ...
          this.payloadQueue = new lib$2('rudder', queueOptions, ((item, done) => {
        ...
        value: function preaparePayloadAndFlush(repo) {
...

          {
            xhr.setRequestHeader(
              'Authorization',
              'Basic '.concat(btoa(''.concat(payload.writeKey, ':'))),
            );
          ...
          xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
              ...
              handleError(
                new Error(
                  'request failed with status: '.concat(xhr.status, ' for url: ').concat(repo.url),
                ),
              );
            }
      ...
      },
      {
        ...
        value: function enqueue(rudderElement, type) {
          ...
          const url = this.url.slice(-1) == '/' ? this.url.slice(0, -1) : this.url; // add items to the queue
  ...
  function shouldTrackElement(el) {
    ...
    while (curEl.parentNode && !isTag(curEl, 'body')) {
      const _classes = getClassName(el).split(' '); // if explicitly specified "rudder-no-track", even at parent level, dont track the child nodes too.

      if (_classes.includes('rudder-no-track')) {
        return false;
      }
    ...
    if (classes.includes('rudder-include')) {
      return true;
    ...
    if (typeof name === 'string') {
      ...
      if (sensitiveNameRegex.test(name.replace(/[^\dA-Za-z]/g, ''))) {
        return false;
      }
    }
        ...
        for (let i = 0; i < target.elements.length; i++) {
          ...
          if (
            ...
            if (name && typeof name === 'string') {
              const key = formElement.id ? formElement.id : formElement.name; // formElement.value gives the same thing
  ...
  function isExplicitNoTrack(el) {
    ...
    if (classes.includes('rudder-no-track')) {
      return true;
    }
  ...
  function isValueToBeTracked(value) {
    ...
    if (typeof value === 'string') {
      value = value.replace(/^\s+|\s+$/g, ''); // check to see if input value looks like a credit card number
  ...
  function getText(el) {
    ...
    el.childNodes.forEach((value) => {
      if (value.nodeType === Node.TEXT_NODE) {
        const textContent = value.nodeValue.replace(/^\s+|\s+$/g, ''); // take each word from the text content and check whether the value should be tracked. Also, replace the whitespaces.
    ...
    _createClass(Analytics, [
      {
        ...
        value: function initializeUser() {
          ...
          this.userTraits =
            this.storage.getUserTraits() != undefined ? this.storage.getUserTraits() : {};
          this.groupId = this.storage.getGroupId() != undefined ? this.storage.getGroupId() : '';
          this.groupTraits =
            this.storage.getGroupTraits() != undefined ? this.storage.getGroupTraits() : {};
        ...
        value: function processResponse(status, response) {
          try {
            ...
            response.source.destinations.forEach(function (destination, index) {
              logger.debug(
                'Destination '
                  .concat(index, ' Enabled? ')
                  .concat(destination.enabled, ' Type: ')
                  .concat(destination.destinationDefinition.name, ' Use Native SDK? ')
                  .concat(destination.config.useNativeSDK),
              );
            ...
            this.clientIntegrations = this.clientIntegrations.filter((intg) => integrations[intg.name] != undefined);
      ...
      },
      {
        ...
        value: function isInitialized(instance) {
          ...
          return new Promise((resolve) => {
            ...
            _this2.pause(INTEGRATION_LOAD_CHECK_INTERVAL).then(() => {
      ...
      },
      {
        ...
        value: function page(category, name, properties, options, callback) {
          ...
          if (_typeof(category) === 'object' && category != null && category != undefined)
            (options = name), (properties = category), (name = category = null);
          if (_typeof(name) === 'object' && name != null && name != undefined)
      ...
      },
      {
        ...
        value: function processAndSendDataToDestinations(type, rudderElement, options, callback) {
          try {
            ...
            const clientSuppliedIntegrations = rudderElement.message.integrations; // get intersection between config plane native enabled destinations
            // (which were able to successfully load on the page) vs user supplied integrations
      ...
      },
      {
        ...
        value: function addCampaignInfo(rudderElement) {
          ...
          const campaign = lib(search);

          if (
            rudderElement.message.context &&
            _typeof(rudderElement.message.context) === 'object'
          ) {
        ...
        value: function processOptionsParam(rudderElement, options) {
          ...
          for (const key in options) {
            if (toplevelElements.includes(key)) {
              ...
              rudderElement.message.context = lodash_merge(
                rudderElement.message.context,
                _defineProperty({}, key, options[key]),
              );
            } else if (_typeof(options[key]) === 'object' && options[key] != null) {
              rudderElement.message.context = lodash_merge(
                rudderElement.message.context,
                _objectSpread2({}, options[key]),
              );
            } else {
              logger.error(
        ...
        value: function load(writeKey, serverUrl, options) {
          ...
          if (options && options.clientSuppliedCallbacks) {
            ...
            Object.keys(this.methodToCallbackMapping).forEach((methodName) => {
              if (_this3.methodToCallbackMapping.hasOwnProperty(methodName) && options.clientSuppliedCallbacks[_this3.methodToCallbackMapping[methodName]]) {
                  tranformedCallbackMapping[methodName] =
                    options.clientSuppliedCallbacks[_this3.methodToCallbackMapping[methodName]];
                }
            });
          ...
          if (
            ...
            _typeof(options.queueOptions) == 'object'
          ) {
      ...
      },
      {
        ...
        value: function registerCallbacks(calledFromLoad) {
          ...
          if (!calledFromLoad) {
            Object.keys(this.methodToCallbackMapping).forEach((methodName) => {
              if (_this5.methodToCallbackMapping.hasOwnProperty(methodName) && window.rudderanalytics && 
                    typeof window.rudderanalytics[_this5.methodToCallbackMapping[methodName]] ===
                    'function'
                  ) {
                    _this5.clientSuppliedCallbacks[methodName] =
                      window.rudderanalytics[_this5.methodToCallbackMapping[methodName]];
                  } // let callback =
      ...
      },
      {
        ...
        value: function parseQueryString(query) {
          ...
          const returnObj = {};
          const queryObject = componentQuerystring.parse(query);
  ...
  if (eventsPushedAlready && argumentsArray && argumentsArray.length > 0) {
    for (const element of argumentsArray) {
      instance.toBeProcessedArray.push(element);
    }
